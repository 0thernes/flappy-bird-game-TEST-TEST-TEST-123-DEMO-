...
function playToneAt(freq, duration, type, volume, startTime) {
  ...
  } catch { /* tone creation failed — swallow gracefully */ }
}
...
function sfxFlap() { 
  playTone(420, 0.10, 'triangle', 0.15); 
  playTone(560, 0.08, 'sine', 0.10); 
}

function sfxScore() { 
  playTone(660, 0.08, 'square', 0.10); 
  playTone(880, 0.10, 'square', 0.12); 
  setTimeout(() => playTone(1100, 0.12, 'square', 0.10), 80); 
}

function sfxDie() { 
  playTone(220, 0.25, 'sawtooth', 0.14); 
  playTone(140, 0.35, 'sawtooth', 0.10); 
}
...
function sfxShieldCollect() {
  ...
  setTimeout(() => playTone(659, 0.12, 'sine', 0.10), 70);
  setTimeout(() => playTone(784, 0.15, 'sine', 0.12), 140);
  setTimeout(() => playTone(1047, 0.20, 'sine', 0.15), 210);
}
...
const musicThemes = {
  sunset: { notes: [262, 330, 392, 523, 392, 330], wave: 'sine', subWave: 'triangle', interval: 0.420 }, // C-Major Cozy
  midnight: { notes: [293, 370, 440, 587, 440, 370], wave: 'square', subWave: 'sine', interval: 0.380 }, // Lydian cyber
  rain: { notes: [220, 262, 330, 440, 330, 262], wave: 'sine', subWave: 'triangle', interval: 0.460 }, // Soothing minor
  aurora: { notes: [277, 311, 349, 415, 349, 311], wave: 'sine', subWave: 'sine', interval: 0.450 }, // Whole-tone cluster
};
...
function scheduleMusicAhead() {
  ...
  while (musicNextNoteTime < audioCtx.currentTime + MUSIC_LOOKAHEAD) {
    ...
    playToneAt(baseFreq / 2, 0.28, theme.subWave, 0.02 * state.musicVolume, musicNextNoteTime);

    // Procedural rain droplets under the Rain theme
    if (state.theme === 'rain' && Math.random() < 0.35) {
      playToneAt(Math.random() * 900 + 350, 0.005, 'sine', 0.007 * state.musicVolume, musicNextNoteTime);
    }
...
const BIRD_THEME_COLORS = {
  sunset: { calm: '#ffd44f', happy: '#ffdd44', scared: '#ffcc44', determined: '#ffaa22', dizzy: '#ddbb44', beak: '#ff8b2d', hl: '#ffe680', shadow: '#f2b638' },
  midnight: { calm: '#f472b6', happy: '#ff77cc', scared: '#c084fc', determined: '#ec4899', dizzy: '#a21caf', beak: '#38bdf8', hl: '#fbcfe8', shadow: '#db2777' },
  rain: { calm: '#94a3b8', happy: '#38bdf8', scared: '#475569', determined: '#1e293b', dizzy: '#64748b', beak: '#2dd4bf', hl: '#cbd5e1', shadow: '#475569' },
  aurora: { calm: '#4ade80', happy: '#a78bfa', scared: '#10b981', determined: '#047857', dizzy: '#34d399', beak: '#facc15', hl: '#bbf7d0', shadow: '#059669' },
};

const GROUND_THEME_COLORS = {
  sunset: { sand: '#c8a24c', grass1: '#5cb85c', grass2: '#3d8b3d', dirt: '#a6893a' },
  midnight: { sand: '#1e0b36', grass1: '#ec4899', grass2: '#9d174d', dirt: '#3c0d63' },
  rain: { sand: '#475569', grass1: '#64748b', grass2: '#334155', dirt: '#1e293b' },
  aurora: { sand: '#0c2e26', grass1: '#10b981', grass2: '#047857', dirt: '#064e3b' },
};
...
function initObjectPools() {
  ...
  for (let i = 0; i < 220; i++) {
    particlePool.push({
      x: 0, y: 0, vx: 0, vy: 0, life: 0, maxLife: 0, color: '', size: 0, active: false
    });
  }
  ...
  for (let i = 0; i < 120; i++) {
    weatherPool.push({
      x: 0, y: 0, vx: 0, vy: 0, type: '', size: 0, length: 0, alpha: 0, active: false
    });
  }
}
...
function updateEmotion() {
  ...
  const closeToPipeEdge = nearPipe && (
  ...
  if (state.phase === 'gameover') {
    bird.emotion = 'dizzy';
  } else if (state.score > 0 && state.score % 5 === 0 && bird.emotionTimer > 0) {
    bird.emotion = 'happy';
...
function resetGame() {
  ...
  // Reset invincibilities and shields
...
function doFlap() {
  ...
  spawnParticles(bird.x - 10, bird.y + 8, 3, particleColorMap[state.theme] || '#ffe680', 2);
}
...
function spawnPipe() {
  ...
  const pipeColorMap = {
    sunset: { color: `hsl(${120 + Math.random() * 30}, 60%, 35%)`, cap: `hsl(${120 + Math.random() * 30}, 70%, 20%)` },
...
function updateBird() {
  ...
  if (bird.isBraking && bird.velocity > 0.45 && state.time - state.lastBrakeToneAt > 14) {
    sfxBrake();
...
function updatePipes() {
  ...
  for (const pipe of pipes) {
    ...
    if (pipe.isMoving) {
      ...
      pipe.topHeight = Math.max(minTop, Math.min(maxTop,
        pipe.baseTopHeight + Math.sin(pipe.movePhase) * 14
      ));
    }
    ...
    if (!pipe.passed && pipe.x + state.pipeWidth < bird.x) {
      ...
      spawnParticles(bird.x, bird.y - 20, 6, scoreColorMap[state.theme] || '#ffd700', 3);
...
function checkCollisions() {
  ...
  for (const pipe of pipes) {
    if (!pipe.passed && Math.abs(pipe.x - bird.x) < 40) {
      const gapBottom = pipe.topHeight + state.gap;
      if ((bird.y - bird.radius < pipe.topHeight + 10 && bird.y - bird.radius > pipe.topHeight - 10) ||
...
function drawBackground() {
  ...
  drawCloud(((70 - co * 0.2) % (canvas.width + 200) + canvas.width + 200) % (canvas.width + 200) - 60, 100, 28);
  drawCloud(((285 - co * 0.15) % (canvas.width + 200) + canvas.width + 200) % (canvas.width + 200) - 60, 150, 22);
  drawCloud(((460 - co * 0.25) % (canvas.width + 200) + canvas.width + 200) % (canvas.width + 200) - 60, 70, 20);
  drawCloud(((180 - co * 0.1) % (canvas.width + 200) + canvas.width + 200) % (canvas.width + 200) - 60, 190, 16);
...
function drawGround() {
  const gConfig = GROUND_THEME_COLORS[state.theme] || GROUND_THEME_COLORS.sunset;
...
function drawBird() {
  ...
  const themeColors = BIRD_THEME_COLORS[state.theme] || BIRD_THEME_COLORS.sunset;

  let bodyColor = themeColors.calm;
  let bodyHL = themeColors.hl;
  
  if (bird.emotion === 'happy') { bodyColor = themeColors.happy; }
  if (bird.emotion === 'scared') { bodyColor = themeColors.scared; }
  if (bird.emotion === 'determined') { bodyColor = themeColors.determined; }
  if (bird.emotion === 'dizzy') { bodyColor = themeColors.dizzy; }
...
function drawScore() {
  const glowColor = bird.emotion === 'happy' ? '#ffd700' :
                    bird.emotion === 'determined' ? '#ff8800' : '#ffffff';
...
function drawPanel(title, subtitle, extra) {
  const pw = 300, ph = 190;
  const px = (canvas.width - pw) / 2, py = 170;
...
function setCustomizerOpen(isOpen, restoreFocus = true) {
  ...
  } catch { /* inert property unsupported */ }
...
function shouldIgnoreGlobalShortcut(event) {
  ...
  return Boolean(target.closest('button, input, select, textarea, [contenteditable="true"], [role="slider"]')) && target !== canvas;
}
...
// (visibilitychange handler registered at the bottom of this file — pauses and persists stats together.)
...
function bindDrawerControls() {
  ...
  dom.gravitySlider?.addEventListener('input', (e) => {
    state.gravitySetting = Math.round(clampNumber(Number(e.target.value), 2, 1, 4));
    if (dom.gravityVal) dom.gravityVal.textContent = GRAVITY_LABELS[state.gravitySetting];
  ...
  dom.speedSlider?.addEventListener('input', (e) => {
    state.speedSetting = Math.round(clampNumber(Number(e.target.value), 2, 1, 4));
    if (dom.speedVal) dom.speedVal.textContent = SPEED_LABELS[state.speedSetting];
    updateDerivedPhysics();
...
// Update visibility change to also save when minimizing/leaving page
